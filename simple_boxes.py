...
def constructChessGrid(gridSize, sizeBox, probability, run, mutation): #probability of being black
    grid = np.array([[[255,255,255,255]]*gridSize]*gridSize)
#    rowIndex1, rowIndex2 = gridSize/2 - sizeBox/2, gridSize/2 +sizeBox/2
#    colIndex1, colIndex2 = gridSize/2 - sizeBox/2, gridSize/2 +sizeBox/2
    ...
    for i in xrange(numBox):
        for j in xrange(numBox):
            ...
            if np.random.uniform() < probability:
                #grid[rowIndex1:rowIndex2, colIndex1: colIndex2] = [[(0,0,0)]*sizeBox]*sizeBox
                grid[rowIndex1:rowIndex2, colIndex1: colIndex2] = [[[0,0,0,255]]*sizeBox]*sizeBox
#    for i in xrange(len(grid)):
#        for j in xrange(len(grid[0])):
#            if np.random.uniform() < mutation:
#                if list(grid[i][j]) == [255,255,255,255]:
#                    grid[i][j] = [0,0,0,255]
#                else:
#                    grid[i][j] = [255,255,255,255]
...
sizeBoxList = [64,32,16,8,4,2,1]#[1,2,4,8,16,32,64]
